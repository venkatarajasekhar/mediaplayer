...
id3v2_get_tag(int fd,int *cancle)
{
	...
	while(lseek(fd, 0L, SEEK_CUR) < header->total_tag_size)
	{
		...
		if( 2!=header->version_minor )
		{
			...
			//printf("The frame id is %c%c%c%c\n",c[0],c[1],c[2],c[3]);
			frame->data_size = id3_32(c, 4);
			//printf("The frame size is %d\n",frame->data_size);
		...
		else
		{
			...
			frame = XMALLOCD(id3v2_frame, "id3v2_get_tag:frame");
			frame->frame_id = xmallocd(3, "id3v2_get_tag:frame->frame_id");
			strncpy(frame->frame_id, c, 3);
			//richard_yu@realsil.com.cn
			frame->data_size = id3_32_V2(c, 3);
		}
		...
		//richard_yu@realsil.com.cn 2010/03/15
		//check if there is error happend here
		
		if( 2!=header->version_minor&& lseek(fd, 0L, SEEK_CUR) < header->total_tag_size )
			{
				
				read(fd, c, 4); 
				
				lseek(fd,-4,SEEK_CUR);
				if(!((isupper(c[0])||isdigit(c[0]))&&
						(isupper(c[1])||isdigit(c[1]))&&
							(isupper(c[2])||isdigit(c[2]))&&
								(isupper(c[3])||isdigit(c[3]))))
					{
						...
						lseek(fd,0-frame->data_size,SEEK_CUR);
						int offset_pos=0;
						char i=0;
						while(offset_pos<frame->data_size)
							{
								read(fd,c,4);
								for(i=0;frame_name[i];i++)
									{
										if(0==strncmp(c,frame_name[i],4))
											break;
								...
								if(frame_name[i])
									{
										xfree(frame->data);
										frame->data_size=offset_pos;
										frame->data = xmallocd(frame->data_size, "id3v2_get_tag:frame->data_size");
										lseek(fd,-4-offset_pos,SEEK_CUR);
										read(fd, frame->data, frame->data_size);
										break;
									}
								else
...
id3v2_get_content_at_pos(id3v2_tag *tag, const char *field, int pos)
{
	...
	for(i = 0, found = 0; frame_list; i++, frame_list = frame_list->next)
	{
		...
		{
			if(strncmp(frame->frame_id, field, 3)) continue;
		}
...
static unsigned int
id3_32(unsigned char* c, int start)
{
...
//richard_yu@realsil.com.cn
static unsigned int
id3_32_V2(unsigned char* c, int start)
{
	return  (c[start+2]) + (c[start+1] << 8) + (c[start] << 16);
}
