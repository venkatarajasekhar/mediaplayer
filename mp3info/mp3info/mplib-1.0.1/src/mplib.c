...
id3_tag_list* 
mp_get_tag_list_from_file(const char* filename,int *cancle)
{
	if(1<=*cancle)
		return NULL;
	id3_tag_list *ret;
	...
	ret = mp_get_tag_list_from_fd(fd,cancle);
	close(fd);
	return ret;
}

id3_tag_list* 
mp_get_tag_list_from_fd(int fd,int *cancle)
{
	if(1 <= *cancle)
		return NULL;
	...
	v2tag = id3v2_get_tag(fd,cancle);
	if(v2tag)
	{
		tag = XMALLOCD0(id3_tag, "mp_get_tag_list_from_fd:tag");
		if(v2tag->header->version_minor == 3 || v2tag->header->version_minor == 4 ||v2tag->header->version_minor == 2)
			tag->version = 2;
		else
			tag->version = -1;
		tag->tag = v2tag;
...
mp_get_content_at_pos(const id3_tag *tag, int field, int pos)
{
	...
	else if(tag->version == 2)
	{
		...
		char *val;
		//richard_yu@realsil.com.cn
		//version 2.3/4
		if(2!=v2->header->version_minor)
		{
			switch(field)
			{
				case MP_ARTIST:
		...
		else
		{
			...
			{
				case MP_ARTIST:
					return mp_get_content_custom_at_pos(tag, "TP1", pos);
				case MP_TITLE:
					return mp_get_content_custom_at_pos(tag, "TT2", pos);
				case MP_ALBUM:
					return mp_get_content_custom_at_pos(tag, "TAL", pos);
				case MP_GENRE:
					return mp_get_content_custom_at_pos(tag, "TCO", pos);
				case MP_COMMENT:
					return mp_get_content_custom_at_pos(tag, "COM", pos);
				case MP_YEAR:
					return mp_get_content_custom_at_pos(tag, "TYE", pos);
				case MP_TRACK:
					return mp_get_content_custom_at_pos(tag, "TRK", pos);
			}
		}
		errno = MP_EFNF;
		return NULL;
 ...
 mp_write_to_fd(const id3_tag_list* tag_list, const int fd)
 {
	 ...
	 while(tag_list)
	 {
		 ...
		 else if(tag->version == 2) 
		 {
			 ...
			 int cancle=1;
			 old_v2 = id3v2_get_tag(fd,&cancle);
			 if(old_v2) {
...
int
mp_get_apic_frame (const id3_tag_list* ls, apic_frame *frame)
{
	id3_tag_list *ptr = ls;
	...
	if(ptr) 
	{
		/* apic frame only exist in v2 tag */
		if(ptr->tag->version == 2) 
		{
			id3v2_frame *frame_data;
			id3v2_frame_list *frame_list;
			id3v2_tag *v2tag;

			v2tag = (id3v2_tag *)ptr->tag->tag;
			frame_list = v2tag->frame_list;

			while(frame_list) 
			{
				id3v2_frame *frame_data = frame_list->data;
				if(frame_data && frame_data->frame_id && strncmp(frame_data->frame_id, "APIC", 4) == 0) 
				{
					char *pivot = frame_data->data;

					frame->encoding = *pivot;
					printf("The encoding is %d\n",frame->encoding);
					pivot++;
					frame->mime_type	= pivot;
					printf("The mime type is %s\n",frame->mime_type);
					pivot+= strlen(pivot) + 1;
					frame->picture_type= *pivot;
					printf("The picture type is %d\n",frame->picture_type);
					pivot++;
					frame->description	= pivot;
					printf("The picture description is %s\n",frame->description);
					pivot+= strlen(pivot) + 1;
					frame->data	= pivot;
					frame->data_size	= frame_data->data_size - (pivot - frame_data->data);
					printf("The picture size is %d\n",frame->data_size);
					return 0;
				}
...
//richard_yu@realsil.com.cn
int SaveThumbnail(const id3_tag_list * list ,char *filename,int size_max,int *cancle)
{
	if(1 <= *cancle)
		return -1;
	if(!list||!filename)
		return -1;
	apic_frame  thunmbnail_frame;

	int ret=-1;
	
	if(-1==mp_get_apic_frame(list ,&thunmbnail_frame))
		return -1;
	
	char * image_name=NULL;
	
	if(thunmbnail_frame.mime_type)
		{
			if(strstr(thunmbnail_frame.mime_type,"jpeg"))
				{
					image_name=malloc(strlen(filename)+strlen("jpg")+1);
					strcpy(image_name,filename);
					strcat(image_name,".jpg");
				}
				else if(strstr(thunmbnail_frame.mime_type,"bmp"))
					{
						image_name=malloc(strlen(filename)+strlen("bmp")+1);
						strcpy(image_name,filename);
						strcat(image_name,".bmp");
					}
					else if(strstr(thunmbnail_frame.mime_type,"png"))
						{
							image_name=malloc(strlen(filename)+strlen("png")+1);
							strcpy(image_name,filename);
	...
	if(strlen(filename)+5<size_max)
		{
			strcat(filename,image_name+strlen(filename));
		}
	else
		return -1;
	...
	fd=open(image_name,O_CREAT|O_WRONLY|O_TRUNC,S_IRWXU);
	...
	write(fd,thunmbnail_frame.data,thunmbnail_frame.data_size);
