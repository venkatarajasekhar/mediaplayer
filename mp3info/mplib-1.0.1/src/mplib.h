...
/** 
 * Retrieve APIC frame from a taglist 
 * @param ls            ptr to a valid id3_tag_list 
 * @return              0 for success, -1 for failure (not found)
 */
extern int mp_get_apic_frame __P((const id3_tag_list* ls, apic_frame *frame));
/**************************************************
 * Save Thumbnail from Id3 tag
 * Arg 1 ---The id3_tag_list
 * Arg 2 ---The thumbnail image name you want to save
 */
extern int SaveThumbnail(const id3_tag_list * list ,char *filename,int size_max,int *cancle);
 ...
 *            been found
 */
extern id3_tag_list* mp_get_tag_list_from_file __P((const char* , int * cancle));
extern id3_tag_list* mp_get_tag_list_from_fd __P((int ,int  * cancle));
