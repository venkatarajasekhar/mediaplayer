...
tag my_udf_query_tag(struct udf_disc *disc, uint16_t Ident, uint16_t SerialNum, uint32_t Location, struct udf_data *data, uint16_t length)
{
	tag ret;
	int i;
	uint16_t crc = 0;
	...
	crc = udf_crc(data->buffer + disc->blocksize * Location + offset, length - offset, crc);
	printf("***%p, %p \n", data->buffer, data->buffer + disc->blocksize * Location);
	printf("Location: %d, crc value: %x \n", Location, crc);
...
int setup_space(struct udf_disc *disc, struct udf_extent *pspace, uint32_t offset)
{
	...
	if (disc->flags & FLAG_SPACE_TABLE) {
		long long rem;
		int max = (0x3FFFFFFF / disc->blocksize) * disc->blocksize;
		int needsize;

		printf("*******************************\n");
		rem = (long long)pspace->blocks * disc->blocksize;
		needsize = (rem / max) * sizeof(short_ad);
		if (rem % max != 0) {
			needsize += sizeof(short_ad);
		}
		printf("needsize: %d \n", needsize);
		if (disc->blocksize - sizeof(struct unallocSpaceEntry) >= needsize) {
			blocknum = 1;
		} else {
			int tempsize = 0;
			needsize -= (disc->blocksize - sizeof(struct unallocSpaceEntry) - 8);
			blocknum = 1;
			tempsize = (disc->blocksize - sizeof(struct allocExtDesc) - 8);
			printf("tempsize: %d \n", tempsize);
			blocknum += (needsize / tempsize);
			if (needsize % tempsize != 0)
				blocknum += 1;
		}
		printf("Using %d block(s) to store unallocated space table...\n", blocknum);
	}
	...
	else if (disc->flags & FLAG_FREED_TABLE)
	{
		...
		{
			phd->freedSpaceTable.extLength = cpu_to_le32(disc->blocksize * blocknum);
			disc->udf_lvid->freeSpaceTable[0] = cpu_to_le32(le32_to_cpu(disc->udf_lvid->freeSpaceTable[0]) - 1 * blocknum);
		}
	}
	...
	else if (disc->flags & FLAG_UNALLOC_TABLE)
	{
		...
		{
			phd->unallocSpaceTable.extLength = cpu_to_le32(disc->blocksize * blocknum);
			disc->udf_lvid->freeSpaceTable[0] = cpu_to_le32(le32_to_cpu(disc->udf_lvid->freeSpaceTable[0]) - 1 * blocknum);
		}
	}
	...
	else if (disc->flags & FLAG_SPACE_TABLE)
	{
		...
		if (disc->flags & FLAG_STRATEGY4096)
			...
			length = disc->blocksize * blocknum;
		desc = set_desc(disc, pspace, TAG_IDENT_USE, offset, disc->blocksize * blocknum, NULL);
		...
		printf("max: %x, short_ad: %d \n", max, sizeof(short_ad));
		printf("value1: %d value2: %d \n", disc->blocksize - sizeof(struct unallocSpaceEntry), (rem / max) * sizeof(short_ad));
//		if (disc->blocksize - sizeof(struct unallocSpaceEntry) < (rem / max) * sizeof(short_ad))
		...
		if (rem > 0x3FFFFFFF)
		{
			...
			while (rem > max)
			{
				...
				if (extoffset + sizeof(short_ad) == disc->blocksize) {
					printf("use->lengthAllocDescs: %d \n", use->lengthAllocDescs);
					flag += 1;
					break;
				}
			}
			...
			while (flag) {
				// fill the link...
				printf("\n2 extoffset: %d \n", extoffset);
				sad->extLength = cpu_to_le32(EXT_NEXT_EXTENT_ALLOCDECS | disc->blocksize);
				sad->extPosition = cpu_to_le32(flag);
				printf("flag value: %d \n", flag);
				if (flag > 1)
					aed->descTag = my_udf_query_tag(disc, TAG_IDENT_AED, 1, desc->offset+flag-1, desc->data, sizeof(struct allocExtDesc) + le32_to_cpu(aed->lengthAllocDescs));

				// fill the allocExtDesc...
				ptr += disc->blocksize;
				aed = (struct allocExtDesc *)ptr;
				aed->lengthAllocDescs = cpu_to_le32(sizeof(short_ad));
				aed->previousAllocExtLocation = cpu_to_le32(flag - 1);

				sad = (short_ad *)(ptr + sizeof(struct allocExtDesc));
				...
				while (rem > max) {
					...
					extoffset += sizeof(short_ad);
					rem -= max;
					aed->lengthAllocDescs = cpu_to_le32(le32_to_cpu(aed->lengthAllocDescs) + sizeof(short_ad));

					if (extoffset + sizeof(short_ad) == disc->blocksize) {
						printf("aed->lengthAllocDescs: %d \n", aed->lengthAllocDescs);
						flag += 1;
						break;
					}
				}
		...
		printf("here...\n");
		if (flag > 0)
			aed->descTag = my_udf_query_tag(disc, TAG_IDENT_AED, 1, desc->offset+flag, desc->data, sizeof(struct allocExtDesc) + le32_to_cpu(aed->lengthAllocDescs));
