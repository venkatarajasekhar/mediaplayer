...
int recvbuf2recvframe(_adapter *padapter, struct recv_buf *precvbuf)
{
	...
	do{		
	...
	#if (LINUX_VERSION_CODE < KERNEL_VERSION(2,6,18)) // http://www.mail-archive.com/netdev@vger.kernel.org/msg17214.html
		pkt_copy = dev_alloc_skb(pkt_offset>2048?pkt_offset:2048);
		pkt_copy->dev = padapter->pnetdev;
	#else			
		pkt_copy = netdev_alloc_skb(padapter->pnetdev, pkt_offset>2048?pkt_offset:2048);
	#endif		
		if(pkt_copy==NULL)
		{
		...
		precvframe->u.hdr.rx_end = pkt_copy->data + (pkt_offset>2048?pkt_offset:2048);
#else
		init_recvframe(precvframe, precvpriv); 
		precvframe->u.hdr.rx_head = precvframe->u.hdr.rx_data = precvframe->u.hdr.rx_tail = pbuf;
		precvframe->u.hdr.rx_end = precvbuf->pend;
...
static int recvbuf2recvframe_u(_adapter *padapter, struct sk_buff *pskb)
{
	...
	do{		
		...
		{	
			...
			precvframe->u.hdr.rx_head = precvframe->u.hdr.rx_data = precvframe->u.hdr.rx_tail = pbuf;
			precvframe->u.hdr.rx_end = pbuf + (pkt_offset>2048?pkt_offset:2048);
		}
